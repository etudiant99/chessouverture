...
class Joueur extends CoupManager
{
    ...
    public function setPseudoImage($id)
    {
        $connecte = $this->connecte();
        if ($connecte == 1)
            $this->_pseudoimage = '<img src="public/images/icons/active.gif">'.$id;
        else
            $this->_pseudoimage = $id;
    }
    ...
    public function setPseudostylise($id)
    {
        ...
        if ($sexe == 'h')
            if ($connecte == 1)
                $style = '<a href="javascript:" onclick="_toggle_help()">'.$this->pseudo().'</a>';
            else
                $style = '<a href="?action=joueur&id='.$this->uid().'"><font class="homme">'.$this->pseudo().'</font></a>';
        else
            if ($connecte == 1)
                $style = '<a href="user_frame.php?uid='.$this->uid().'" rel="superbox[iframe.wikipedia][454x380]"><img src="public/images/icons/active.gif" border="0"><font class="femme">'.$this->pseudo().'</font></a>';
            else
    ...
    public function setDestinatairestylise($id)
    {
        ...
        if ($sexe == 'h')
            if ($connecte == 1)
                $infos = '<a href="user_frame.php?uid='.$this->uid().'" rel="superbox[iframe.wikipedia][454x380]"><img src="public/images/icons/active.gif" border="0"><font class="homme">'.$this->pseudo().'</a></font>';
            else
                $infos = '<a href="user_frame.php?uid='.$this->uid().'" rel="superbox[iframe.wikipedia][454x380]"><font class="homme">'.$this->pseudo().'</font></a>';
        else
            if ($connecte == 1)
                $infos = '<a href="user_frame.php?uid='.$this->uid().'" rel="superbox[iframe.wikipedia][454x380]"><img src="public/images/icons/active.gif" border="0"><font class="femme">'.$this->pseudo().'</font></a>';
            else
    ...
    {
        $this->_sexe = $id;
        $this->setPseudoImage($this->pseudo());
    ...
    public function setPhoto($id)
    {
        ...
        if ($presente == 'o')
        {
            ...
            $laphoto = './public/images/joueurs/h'.$this->uid().'.jpg?'.$nombre_aleatoire; // pour recharger la véritable photo;
        }
        else
            $laphoto = "./public/images/joueurs/h0.jpg";
    ...
    public function pseudoimage()
    {
        return $this->_pseudoimage;
    }
